...
fn parse_multicall_args(
  args: impl IntoIterator<Item = String>,
) -> Result<Cli, pound::Error> {
...
fn eval_nix_config(config_path: &Path, impure: bool) -> Result<Value, String> {
  ...
  let output = cmd.output().map_err(|e| {
    format!(
      "failed to execute 'nix eval' for config '{}': {e}. Ensure Nix is \
  ...
  if !output.status.success() {
    return Err(format!(
      "nix eval failed for config '{}': {}\nHint: ensure the file evaluates \
       to a manifest value or {{ manifest = ...; }} and pass --impure if your \
       expression needs impure builtins",
      config_path.display(),
...
fn eval_nix_flake(
  ...
  let output = cmd.output().map_err(|e| {
    format!(
      "failed to execute 'nix eval' for flake '{flake_ref}': {e}. Ensure Nix \
       is installed and available in PATH"
    )
  })?;
  if !output.status.success() {
    return Err(format!(
      "nix eval failed for flake '{}': {}\nHint: verify the flake output attr \
       '{}' exists and evaluates to a manifest",
      flake_ref,
      String::from_utf8_lossy(&output.stderr),
      attr
    ));
...
fn run_cleanup_state(
  ...
  let enabled: BTreeSet<&str> =
    enabled_users.iter().map(String::as_str).collect();
...
fn install_package_profile(
  ...
  let output = cmd.output().map_err(|e| {
    format!(
      "failed to execute 'nix profile install' for standalone packages: {e}. \
       Ensure Nix is installed and available in PATH"
    )
  })?;
  if output.status.success() {
    Ok(())
...
fn set_current_package_profile(
  base: &Path,
  generation: &str,
) -> Result<(), String> {
  ...
  std::os::unix::fs::symlink(&generation_profile, &tmp)
    .map_err(|e| e.to_string())?;
...
fn parse_expire_timestamp(input: &str) -> Result<u64, String> {
  ...
  Err(format!(
    "Unsupported timestamp format '{input}'. Use unix seconds, YYYY-MM-DD, \
     RFC3339, or -N days"
  ))
}
...
fn timestamp_to_seconds(timestamp: i64, input: &str) -> Result<u64, String> {
  u64::try_from(timestamp)
    .map_err(|_| format!("Timestamp is before epoch: {input}"))
}
...
fn set_current_generation_id(base: &Path, id: &str) -> Result<(), String> {
  ...
  let current_tmp =
    base.join(format!(".current-generation-{}", std::process::id()));
...
fn sort_generation_ids(ids: &mut [String]) {
  ids.sort_by(|a, b| {
    match (generation_order_key(a), generation_order_key(b)) {
...
mod tests {
  ...
  fn generation_paths_reject_invalid_ids() {
    assert!(
      generation_manifest_path(Path::new("/state"), "../../outside").is_err()
    );
    assert!(
      generation_manifest_path(Path::new("/state"), "generation-1-2-extra")
        .is_err()
    );
